...
class Joint2Odrive(Node):
    def __init__(self):
        ...
        self.declare_parameter('axis_indices', list(range(1)))   # which 12 axes exist (0..11)
        self.declare_parameter('motor_map',    list(range(1)))   # joint index -> axis index mapping
        self.declare_parameter('gear_ratio',   [13.0])          # motor_rot / joint_rot
        self.declare_parameter('sign',         [1.0])          # +1 or -1 per axis
        self.declare_parameter('zero_rad',     [0.0])          # joint zero offsets (rad)
        self.declare_parameter('offset_rotation',   [0.0])          # constant motor rotation offsets
        ...
        self.declare_parameter('axis_state', [8])                 # Sets motor to closed loop control
        ...
        self.axis_state   = int(self.get_parameter('axis_state').value)
